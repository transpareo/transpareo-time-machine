//
// VENDORED THIRD-PARTY CODE - do not edit or reformat.
//
// noble-ed25519 v3.1.0, copied verbatim from the npm package
// (https://github.com/paulmillr/noble-ed25519). Licensed MIT
// (banner below; full license text in
// THIRD-PARTY-LICENSES.md), NOT GPL-3.0 like the rest of
// this repo; kept byte-for-byte so it stays auditable
// against upstream. The build re-applies the banner to the
// emitted chunk so published artefacts retain the notice.
//
// Used only as the pure-JS Ed25519 verify fallback for
// browsers without native WebCrypto Ed25519 (see
// src/crypto/verify.ts). It is lazily imported, so engines
// with native support never download it. SHA-512 is taken
// from crypto.subtle (noble's default async path), which is
// available even where native Ed25519 is not.
//
